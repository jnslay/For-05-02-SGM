...
<!--  Last Published: Mon Apr 29 2024 19:16:53 GMT+0000 (Coordinated Universal Time)  -->
...
$sql4 = "SELECT `Surgeon 1` FROM `team information` WHERE 1";
$result4 = $con->query($sql4);
if ($result4->num_rows > 0) {
    $row = $result4->fetch_assoc();
    $surgeon = $row["Surgeon 1"];
}

$sql5 = "SELECT `Circulating Nurse` FROM `team information` WHERE 1";
$result5 = $con->query($sql5);
if ($result5->num_rows > 0) {
    $row = $result5->fetch_assoc();
    $rn = $row["Circulating Nurse"];
}

$sql6 = "SELECT `Scrub Tech` FROM `team information` WHERE 1";
$result6 = $con->query($sql6);
if ($result6->num_rows > 0) {
    $row = $result6->fetch_assoc();
    $scrubTech = $row["Scrub Tech"];
}

$sql7 = "SELECT `Anesthesiologist` FROM `team information` WHERE 1";
...
$sql8 = "SELECT `Surgeon 2` FROM `team information` WHERE 1";
$result8 = $con->query($sql8);
if ($result8->num_rows > 0) {
    $row = $result8->fetch_assoc();
    $assistant = $row["Surgeon 2"];
}
...
$sql10 = "SELECT `Location` FROM `procedure information` WHERE 1";
...
$sql11 = "SELECT `Booking Start Time` FROM `procedure information` WHERE 1";
$result11 = $con->query($sql11);
if ($result11->num_rows > 0) {
    $row = $result11->fetch_assoc();
    $bookingTime = $row["Booking Start Time"];
}

$sql12 = "SELECT `Anticipated Start Time` FROM `procedure information` WHERE 1";
$result12 = $con->query($sql12);
if ($result12->num_rows > 0) {
    $row = $result12->fetch_assoc();
    $startTime = $row["Anticipated Start Time"];
}

 ?>


<html data-wf-page="662eeb77747f310d5ccd4729" data-wf-site="6629b58137d7c761e37bacd0">
<head>
  <meta charset="utf-8">
  <title>info confirmation</title>
  <meta content="info confirmation" property="og:title">
  <meta content="info confirmation" property="twitter:title">
      ...
      <div class="w-col w-col-6">
        <div class="w-layout-blockcontainer container-6 w-container">
          <div class="w-layout-hflex">
            <h1 class="heading-11">Patient Information</h1><img src="images/patient-picture-placeholder.png" loading="lazy" sizes="100px" srcset="images/patient-picture-placeholder-p-500.png 500w, images/patient-picture-placeholder.png 626w" alt="" class="image-3">
          </div>
          <div class="w-row">
            <div class="column-6 w-col w-col-6">
              <div class="text-block-3">Name: <?php echo $name ?></div>
              <div class="text-block-4">DOB: <?php echo $dob ?></div>
            </div>
            <div class="column-7 w-col w-col-6">
              <div class="text-block-6">MRN <?php echo $mrn ?> </div>
            </div>
          </div>
          <div class="text-block-10">Allergies: <?php echo $allergies ?></div>
        </div>
        <div class="w-layout-blockcontainer container-7 w-container">
          <h1 class="heading-12">Procedure</h1>
          <p class="paragraph"> <?php echo $procedure ?></p>
        </div>
        <div class="w-layout-blockcontainer container-8 w-container">
          <h1 class="heading-13">Scheduling</h1>
          <div class="text-block-7">Location: <?php echo $location ?></div>
          <div class="text-block-8">Booking Start Time: <?php echo $bookingTime ?> </div>
          <div class="text-block-9">Anticipated Procedure Start Time: <?php echo $startTime ?> </div>
        </div>
      </div>
      <div class="w-col w-col-6">
        <div class="w-layout-blockcontainer container-9 w-container">
          <h1 class="heading-14">Surgical Team</h1>
          <div class="w-layout-hflex flex-block"><img src="images/surgeon-placeholder.png" loading="lazy" alt="" class="image-4">
            <div class="text-block-11">Surgeon: <?php echo $surgeon ?> </div>
          </div>
          <div class="w-layout-hflex flex-block-2"><img src="images/surgeon-placeholder.png" loading="lazy" alt="" class="image-4">
            <div class="text-block-11">Anesthesiologist: <?php echo $anesthesiologist ?></div>
          </div>
          <div class="w-layout-hflex flex-block-3"><img src="images/surgeon-placeholder.png" loading="lazy" alt="" class="image-4">
            <div class="text-block-11">Assistant: <?php echo $assistant ?></div>
          </div>
          <div class="w-layout-hflex flex-block-3"><img src="images/surgeon-placeholder.png" loading="lazy" alt="" class="image-4">
            <div class="text-block-11">Scrub Tech: <?php echo $scrubTech ?></div>
          </div>
          <div class="w-layout-hflex flex-block-3"><img src="images/surgeon-placeholder.png" loading="lazy" alt="" class="image-4">
            <div class="text-block-11">RN: <?php echo $rn ?></div>
          </div>
        </div>
        <div class="w-layout-blockcontainer w-container">
          <a href="pref-card-search.php" class="confirm-button w-button">Confirm</a>
        </div>
      </div>
    </div>
  </section>
